...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const r = await fetch(`https://www.pay.moneyfusion.net/paiementNotif/${token}`);
    const data = await r.json();

    const statut = data?.data?.statut;
    const { data: deposit } = await admin
      .from("deposits").select("*").eq("reference", token).maybeSingle();
    ...
    } else if (deposit && deposit.status === "pending" && (statut === "failure" || statut === "no paid")) {
