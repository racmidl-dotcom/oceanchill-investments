      ...
      <div className="px-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-panel text-panel-foreground rounded-md p-4">
            ...
            <p className="text-2xl font-serif font-semibold mt-2">
              {data.length}
            </p>
          </div>
          <div className="bg-panel text-panel-foreground rounded-md p-4">
        ...
        <div className="space-y-3 pt-2">
          ...
          {!loading && data.length === 0 && (
            <div className="text-center py-16">
              <Refrigerator
                className="w-14 h-14 mx-auto text-muted-foreground/40"
                strokeWidth={1.2}
              />
              <p className="text-foreground mt-4 font-serif">
                Vous n'avez pas acheté de produit
              </p>
            </div>
          )}
          {data.map((inv) => {
            ...
            return (
              <div
                key={inv.id}
                className="bg-card border border-border/40 rounded-md p-4"
              >
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Fin : {new Date(inv.end_date).toLocaleDateString("fr-FR")}
                </p>
                <div className="flex justify-between text-xs mt-2">
                  <span>
                    Gains :{" "}
                    <strong className="text-panel-dark">
                      {formatMoney(earned, cur)}
                    </strong>
                  </span>
                  <span>
                    Total : <strong>{formatMoney(total, cur)}</strong>
                  </span>
                </div>
                <div className="h-1.5 bg-border rounded-full mt-2 overflow-hidden">
                  <div
                    className="h-full bg-panel-dark"
                    style={{ width: `${pct}%` }}
                  />
                </div>
              </div>
